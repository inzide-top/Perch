...
test('全局聊天 Prompt 使用 PERCH 产品身份且不冒充已执行工具', () => {
  ...
  assert.match(prompt, /终止、放弃或关闭某个机会.*terminate_opportunity/)
  ...
  assert.match(prompt, /多个独立操作时，必须逐项核对/)
  assert.match(prompt, /本轮任务必须分别调用 update_opportunity_profile 和 transition_opportunity_status/)
  assert.match(prompt, /任一工具未成功时不得声称两项均已完成/)
})
...
test('机会聊天 Prompt 注入绑定机会并明确禁止混入其他机会', () => {
  ...
  assert.match(prompt, /终止、放弃或关闭当前机会.*terminate_opportunity/)
  ...
  assert.match(prompt, /基础资料修改和阶段流转时.*分别调用 update_opportunity_profile 与 transition_opportunity_status/)
  assert.match(prompt, /只收到其中一个工具成功结果时声称另一项也已完成/)
